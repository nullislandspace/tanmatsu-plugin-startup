...
static void decode_loop(void) {
    ...
    while (g_playing && !g_thread_should_stop) {
        ...
        if (available < 4) {
            ...
            g_finished = true;
            g_playing = false;
            asp_log_info("startup", "Audio playback finished");
            break;
        }
        ...
        if (samples > 0) {
            ...
            if (!g_format_logged) {
                asp_log_info("startup", "Format: %d Hz, %d ch, %d kbps",
            ...
            if (fill_buffer() == 0) {
                ...
                g_finished = true;
                g_playing = false;
                asp_log_info("startup", "Audio playback finished (no more data)");
                break;
            }
        }
    }
...
static void start_new_file(const char* path) {
    ...
    if (!g_current_file) {
        asp_log_error("startup", "Failed to open: %s", path);
        g_playing = false;
        g_finished = true;
        return;
    }
    ...
    asp_log_info("startup", "Playing: %s", path);
}

// Decoder thread main function
static void* decoder_thread_func(void* arg) {
    (void)arg;
    asp_log_info("startup", "Decoder thread started");
    ...
    asp_log_info("startup", "Decoder thread exiting");
...
int audio_init(void) {
    if (g_audio_initialized) {
        asp_log_warn("startup", "Audio already initialized");
        audio_cleanup();
    }

    asp_log_info("startup", "Allocating audio buffers...");

    read_buffer = (uint8_t*)malloc(READ_BUFFER_SIZE);
    if (!read_buffer) {
        asp_log_error("startup", "Failed to allocate read_buffer");
        return -1;
    }

    g_mp3_decoder = (mp3dec_t*)malloc(sizeof(mp3dec_t));
    if (!g_mp3_decoder) {
        asp_log_error("startup", "Failed to allocate mp3_decoder");
        free(read_buffer);
        read_buffer = NULL;
        return -1;
    }
    ...
    if (err != 0) {
        asp_log_error("startup", "Failed to create decoder thread: %d", err);
        free(read_buffer);
        free(g_mp3_decoder);
        read_buffer = NULL;
        g_mp3_decoder = NULL;
    ...
    if (prio_err != 0) {
        asp_log_warn("startup", "Failed to raise decoder priority: %d", prio_err);
    }

    g_thread_running = true;
    g_audio_initialized = true;

    asp_log_info("startup", "Audio initialized");
    return 0;
}
...
void audio_cleanup(void) {
    ...
    asp_log_info("startup", "Audio cleanup starting...");
    ...
    asp_log_info("startup", "Audio cleanup complete");
}
